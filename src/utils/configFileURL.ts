...
export const reConfigFileURL =
  /http[s]?:\/\/(\w+)\.com\/([^/\s]+)\/([^/\s]+)\/([^/\s]+)\/([^/\s]+)\/([\S]+)/g;
...
export const validateConfigFileURL = ({
  platform,
  owner,
  repo,
  type,
  ...
  if (
    ConfigFilePlatformEnum.GITEE !== platform &&
    ConfigFilePlatformEnum.GITHUB !== platform
  )
    throw new Error('Error');

  if (type !== 'blob') throw new Error('Error');
};

export const fetchConfigFileContent = async ({
  platform,
  owner,
  repo,
  ref,
  path,
}: ConfigFileInfo): Promise<string> => {
  const URL =
    ConfigFilePlatformEnum.GITHUB === platform
      ? `https://api.github.com/repos/${owner}/${repo}/contents/${path}?ref=${ref}`
      : `https://gitee.com/api/v5/repos/${owner}/${repo}/contents/${path}?ref=${ref}`;
